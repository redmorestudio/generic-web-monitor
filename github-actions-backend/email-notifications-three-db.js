  ...
  async sendNoChangesEmail() {
    try {
      ...
      const stats = {
        ...
        nextRun: new Date(Date.now() + 6 * 60 * 60 * 1000).toLocaleString() // 6 hours later
      };

      // Get list of monitored companies
      const companies = this.intelligenceDb.prepare(`
        SELECT name, category FROM companies ORDER BY name
      `).all();
      ...
      let html = `
        ...
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px;">
          <p style="margin: 5px 0;">🏢 <strong>${stats.totalCompanies} Companies</strong> monitored</p>
          <p style="margin: 5px 0;">🔗 <strong>${stats.totalUrls} URLs</strong> checked</p>
          <p style="margin: 5px 0;">🔄 <strong>${recentChanges} changes</strong> detected (all low priority)</p>
          <p style="margin: 5px 0;">⏰ <strong>Last check:</strong> ${stats.lastRun}</p>
          <p style="margin: 5px 0;">⏰ <strong>Next check:</strong> ${stats.nextRun}</p>
        </div>
  ...
  async checkAndAlert() {
    try {
      ...
      if (recentHighPriorityChanges.length > 0) {
        ...
        const anyChanges = this.processedDb.prepare(`
          SELECT COUNT(*) as count
          FROM change_detection 
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log('  node email-notifications-three-db.js nochanges   - Send no-changes notification');

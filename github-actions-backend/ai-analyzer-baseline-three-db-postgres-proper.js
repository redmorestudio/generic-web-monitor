...
async function storeBaselineAnalysis(company, url, extractedData) {
  try {
    // Get company_id and url_id
    const companyRecord = await db.get(
      'SELECT id FROM intelligence.companies WHERE name = $1',
      [company]
    );
    
    if (!companyRecord) {
      console.error(`   ❌ Company not found in database: ${company}`);
    ...
    if (!urlRecord) {
      console.error(`   ❌ URL not found in database: ${url}`);
      // Create URL if it doesn't exist
      const newUrl = await db.get(
        'INSERT INTO intelligence.company_urls (company_id, url, name, category) VALUES ($1, $2, $3, $4) RETURNING id',
        [companyRecord.id, url, new URL(url).pathname || 'Home', 'main']
      );
      urlRecord = newUrl;
    }
    ...
    // Prepare data for insertion into JSONB structure
    const summary = extractedData.summary?.one_line || 
      `${company}: ${extractedData.current_state?.positioning || 'AI company'}`;
    
    // Extract themes (convert objects to strings if needed)
    const themes = extractedData.summary?.key_insights || [];
    const themesArray = themes.map(t => typeof t === 'object' ? JSON.stringify(t) : t);
    
    // Extract sentiment
    const sentiment = {
      overall: extractedData.strategic_intelligence?.interest_assessment?.category || 'neutral',
      score: extractedData.strategic_intelligence?.interest_assessment?.interest_level || 5,
      technical_score: extractedData.strategic_intelligence?.interest_assessment?.technical_innovation_score || 5,
      business_score: extractedData.strategic_intelligence?.interest_assessment?.business_impact_score || 5
    };
    
    // Extract key points
    const keyPoints = [
      ...(extractedData.summary?.key_insights || []),
      ...(extractedData.summary?.notable_facts || [])
    ].filter(Boolean);

    // Store using PostgreSQL syntax with ON CONFLICT
    await db.run(`
      INSERT INTO intelligence.baseline_analysis 
      (company_id, company_name, url_id, url, 
       entities, themes, sentiment, key_points, relationships,
       summary, company_type, page_purpose, 
       key_topics, main_message, target_audience, unique_value,
       trust_elements, differentiation, technology_stack,
       analysis_date, content_hash, ai_model)
    ...
    `, [
      ...
      urlRecord.id,
      url,
      JSON.stringify(extractedData.entities || {}),           // JSONB entities
      JSON.stringify(themesArray),                            // JSONB themes  
      JSON.stringify(sentiment),                              // JSONB sentiment
      JSON.stringify(keyPoints),                              // JSONB key_points
      JSON.stringify(extractedData.relationships || []),      // JSONB relationships
      summary,                                                // text summary
      extractedData.entities?.products?.[0]?.type || 'AI Company',
      extractedData.current_state?.positioning || '',
      extractedData.summary?.key_insights || [],              // text[] key_topics
      extractedData.summary?.one_line || '',
      extractedData.entities?.markets?.[0]?.segment || '',
      extractedData.current_state?.value_props?.[0] || '',
      extractedData.current_state?.core_capabilities || [],   // text[] trust_elements
      extractedData.strategic_intelligence?.innovation_level || '',
      extractedData.entities?.technologies?.map(t => t.name) || [], // text[] technology_stack
...
async function generateBaselineReport() {
  console.log('\n📊 Generating baseline intelligence report (PostgreSQL with JSONB)...');
  ...
  for (const company of companies) {
    ...
    for (const analysis of analyses) {
      // Extract from JSONB entities
      if (analysis.entities) {
        const entities = typeof analysis.entities === 'string' ? 
          JSON.parse(analysis.entities) : analysis.entities;
        
        if (entities.products) {
          aggregated.products.push(...entities.products);
        }
        if (entities.technologies) {
          aggregated.technologies.push(...entities.technologies);
        }
        if (entities.partnerships) {
          aggregated.partnerships.push(...entities.partnerships);
        }
        if (entities.integrations) {
          aggregated.integrations.push(...entities.integrations);
        }
      }
      ...
      if (analysis.themes) {
        ...
        aggregated.themes.push(...themes);
      }
      
      // Extract relationships
      if (analysis.relationships) {
        const relationships = typeof analysis.relationships === 'string' ? 
          JSON.parse(analysis.relationships) : analysis.relationships;
      ...
      if (analysis.sentiment) {
        const sentiment = typeof analysis.sentiment === 'string' ? 
          JSON.parse(analysis.sentiment) : analysis.sentiment;
        sentimentSum += sentiment.score || 5;
      }
      ...
      if (analysis.key_points) {
        ...
        aggregated.key_insights.push(...keyPoints);
      }
    }

    // Calculate averages and deduplicate
    aggregated.sentiment_avg = analyses.length > 0 ? 
      (sentimentSum / analyses.length).toFixed(2) : 0;
    
    // Deduplicate arrays by name/partner/concept
    aggregated.products = [...new Map(aggregated.products.map(p => [p.name, p])).values()];
    aggregated.technologies = [...new Map(aggregated.technologies.map(t => [t.name, t])).values()];
    aggregated.partnerships = [...new Map(aggregated.partnerships.map(p => [p.partner, p])).values()];
    aggregated.integrations = [...new Map(aggregated.integrations.map(i => [i.name, i])).values()];
    aggregated.themes = [...new Set(aggregated.themes)];
    aggregated.key_insights = [...new Set(aggregated.key_insights)];

    // Update statistics
    report.statistics.total_products += aggregated.products.length;
    report.statistics.total_technologies += aggregated.technologies.length;
    report.statistics.total_partnerships += aggregated.partnerships.length;
    report.statistics.total_integrations += aggregated.integrations.length;
    report.statistics.total_relationships += aggregated.relationships.length;
  ...
  console.log(`   - Total products: ${report.statistics.total_products}`);
  console.log(`   - Total technologies: ${report.statistics.total_technologies}`);
  console.log(`   - Total partnerships: ${report.statistics.total_partnerships}`);
  console.log(`   - Total integrations: ${report.statistics.total_integrations}`);
  console.log(`   - Total relationships: ${report.statistics.total_relationships}`);
...
if (require.main === module) {
  processAllSnapshots()
    .then((report) => {
      ...
      console.log('🔗 Rich JSONB data stored for dashboard and 3D visualization');

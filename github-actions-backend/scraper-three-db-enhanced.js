...
class IntelligentScraperThreeDB {
  ...
  async initialize() {
    console.log('🚀 Starting Enhanced Intelligent Scraper with Captcha Detection...');
    ...
    // Run database migrations to ensure we have the new columns
  ...
  async runMigrations() {
    ...
    try {
      // Add scrape_status column to raw_html if it doesn't exist
      const rawTableInfo = this.rawDb.pragma('table_info(raw_html)');
      const hasScrapeStatus = rawTableInfo.some(col => col.name === 'scrape_status');
      
      if (!hasScrapeStatus) {
        this.rawDb.exec(`
          ALTER TABLE raw_html ADD COLUMN scrape_status TEXT DEFAULT 'success';
          CREATE INDEX IF NOT EXISTS idx_raw_html_status ON raw_html(scrape_status);
        `);
        console.log('✅ Added scrape_status column to raw_html');
      }

      // Add interest_level and interest_data columns to change_detection if they don't exist
      const processedTableInfo = this.processedDb.pragma('table_info(change_detection)');
      const hasInterestLevel = processedTableInfo.some(col => col.name === 'interest_level');
      const hasInterestData = processedTableInfo.some(col => col.name === 'interest_data');
      
      if (!hasInterestLevel) {
        this.processedDb.exec(`
        ...
        `);
        console.log('✅ Added interest_level column to change_detection');
      }
      ...
      if (!hasInterestData) {
        ...
        `);
        console.log('✅ Added interest_data column to change_detection');
      }
  ...
  async scrapeUrl(urlConfig, companyName) {
    ...
    try {
      ...
      // Check if content has changed (exclude blocked pages from comparison)
      const latestStmt = this.rawDb.prepare(`
        SELECT id, content_hash, html_content FROM raw_html
        WHERE url_id = ? AND scrape_status != 'blocked'

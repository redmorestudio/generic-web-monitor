    ...
    try {
        ...
        for (const change of changes) {
            ...
            try {
                ...
                const timestamp = new Date(change.detected_at).toISOString().replace(/[:.]/g, '-');
                const filename = `${change.company_name.toLowerCase().replace(/[^a-z0-9]/g, '-')}-${timestamp.substring(0, 19)}.json`;
